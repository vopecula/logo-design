  ...
  switch (type) {
    case Face.LEFT:
      ...
      OZ.CSS3.set(
        this._node,
        "transform",
        "translate3d(-" + Face.SIZE + "px, 0px, 0px) rotateY(-90deg)"
      );
      break;
    case Face.RIGHT:
    ...
    case Face.TOP:
      ...
      OZ.CSS3.set(
        this._node,
        "transform",
        "translate3d(0px, -" + Face.SIZE + "px, 0px) rotateX(90deg)"
      );
      break;
    case Face.BOTTOM:
    ...
    case Face.BACK:
      OZ.CSS3.set(
        this._node,
        "transform",
        "translate3d(0px, 0px, -" + Face.SIZE + "px) rotateY(180deg)"
      );
      break;
  }
};
...
Cube.prototype._update = function () {
  ...
  var val = this._rotation ? prop + " 500ms" : "";
...
Rubik.prototype.init = function () {
  ...
  this._rotation = Quaternion.fromRotation([1, 0, 0], -35).multiply(
    Quaternion.fromRotation([0, 1, 0], 45)
  );
  this._node = OZ.DOM.elm("div", {
    position: "absolute",
    left: "50%",
  ...
  setTimeout(this.randomize.bind(this), turnDuration);
};
...
Rubik.prototype._rotateY = function (dir, layer) {
  ...
  for (var i = 0; i < Rubik.SIZE; i++) {
    for (var j = 0; j < Rubik.SIZE; j++) {
      cubes.push(
        this._cubes[j + layer * Rubik.SIZE + i * Rubik.SIZE * Rubik.SIZE]
      );
    }
  }
  ...
  for (var z = 0; z < Rubik.SIZE; z++) {
    for (var y = 0; y < Rubik.SIZE; y++) {
      for (var x = 0; x < Rubik.SIZE; x++) {
        ...
        if (z == 0) {
          cube.setFace(Face.FRONT, primaryColor);
        }
        if (z == 2) {
          cube.setFace(Face.BACK, primaryColor);
        }

        if (x == 0) {
          cube.setFace(Face.LEFT, primaryColor);
        }
        if (x == 2) {
          cube.setFace(Face.RIGHT, secondaryColor);
        }

        if (y == 0) {
          cube.setFace(Face.TOP, secondaryColor);
        }
        if (y == 2) {
          cube.setFace(Face.BOTTOM, secondaryColor);
        }
